...
$('.animation__play').click(function (event) {
let t = $(this);
t.parents('.animation').addClass('active');
t.hide();

let c = t.parents('.animation').find('.animation__circle');

c.on("animationend", function () {
    t.parents('.animation').removeClass('active');
    t.show();
});
});
...
code.forEach(codeItem => {
    let codeItems = codeItem.innerHTML;
    codeItems = codeItems.replace(/</g, "&lt;");
    codeItems = codeItems.replace(/>/g, "&gt;");
    codeItem.innerHTML = codeItems;
});
}
